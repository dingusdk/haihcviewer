...
class IHCGroup extends IHCBase {
  ...
  constructor(node: Node) {
    ...
    this.FindAndAdd(node, "functionblock", (subnode: Node) => {
...
class IHCProduct extends IHCBase {
  ...
  constructor(node: Node) {
    ...
    this.FindAndAdd(node, "airlink_input", (subnode: Node) => {
      this.Inputs.push(new IHCInput(subnode));
    });
    this.FindAndAdd(node, "airlink_dimmer_increase", (subnode: Node) => {
      this.Inputs.push(new IHCInput(subnode));
    });
    this.FindAndAdd(node, "airlink_dimmer_decrease", (subnode: Node) => {
      this.Inputs.push(new IHCInput(subnode));
    });
    this.FindAndAdd(node, "airlink_dimming", (subnode: Node) => {
      this.Inputs.push(new IHCInput(subnode));
    });
    this.FindAndAdd(node, "dataline_input", (subnode: Node) => {
      this.Inputs.push(new IHCInput(subnode));
    });

    this.FindAndAdd(node, "airlink_relay", (subnode: Node) => {
      this.Outputs.push(new IHCOutput(subnode));
    });
    this.FindAndAdd(node, "dataline_output", (subnode: Node) => {
      this.Outputs.push(new IHCOutput(subnode));
    });
    this.FindAndAdd(node, "light_indication", (subnode: Node) => {
      this.Outputs.push(new IHCOutput(subnode));
    });
  }
}
